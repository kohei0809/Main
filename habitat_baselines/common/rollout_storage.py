...
class RolloutStorageOracle:
    ...
    def recurrent_generator(
        ...
        #logger.info("############# Start recurrent_generator ###############")
        num_environments = advantages.size(1)
        #logger.info(f"############## num_environments={num_environments} ##############")
        ...
        for inds in torch.randperm(num_environments).chunk(num_mini_batch):
            #logger.info("######## Start for in RolloutStorageOracle ###########")
            #logger.info(f"######## size=({len(self.buffers)}, {len(self.buffers[0])}), current_rollout_step_idx={self.current_rollout_step_idx}, inds={inds}")
            #logger.info(self.buffers)
            #logger.info(f"######## current_rollout_step_idx={self.current_rollout_step_idx}, inds={inds}")
            #for key, value in self.buffers.items():
                #logger.info(f"key={key}, value_size={len(value)}")
            batch = self.buffers[0 : self.current_rollout_step_idx, inds]
            #logger.info("########### After Batch ##########")
            batch["advantages"] = advantages[
                0 : self.current_rollout_step_idx, inds
            ]
            #logger.info("########## After advantages ############")
            batch["recurrent_hidden_states"] = batch[
                "recurrent_hidden_states"
            ][0:1]
            #logger.info("########### After recurrent_hiden_states ############")
