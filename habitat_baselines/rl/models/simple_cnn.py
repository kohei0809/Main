    ...
    def __init__(self, observation_space, output_size):
        ...
        if self._n_input_rgb > 0:
            cnn_dims = np.array(
                ...
                observation_space.spaces["rgb"].shape[:2], dtype=np.float64
            )
        elif self._n_input_depth > 0:
            cnn_dims = np.array(
                #observation_space.spaces["depth"].shape[:2], dtype=np.float32
                observation_space.spaces["depth"].shape[:2], dtype=np.float64
            )
        ...
        if self.is_blind:
            ...
            for kernel_size, stride in zip(
                ...
                cnn_dims = self._conv_output_dim(
                    ...
                    padding=np.array([0, 0], dtype=np.float64),
                    #dilation=np.array([1, 1], dtype=np.float32),
                    dilation=np.array([1, 1], dtype=np.float64),
                    #kernel_size=np.array(kernel_size, dtype=np.float32),
                    kernel_size=np.array(kernel_size, dtype=np.float64),
                    #stride=np.array(stride, dtype=np.float32),
                    stride=np.array(stride, dtype=np.float64),
                )
...
class MapCNN(nn.Module):
    ...
    def __init__(self, map_size, output_size, agent_type):
        ...
        cnn_dims = np.array(
            ...
            [map_size, map_size], dtype=np.float64
        )
